This repository contains the anbound-lock firmware project. It's an open source project that contains firmware to turn any cc2541 based lock into a timer lock.
...
What do you need to build and use this:
 ...
 - An anbound brand lock, for example: 
   https://nl.aliexpress.com/item/4001122501698.html
 - Or a M1L lock, for example: https://www.ebay.nl/itm/402719272973
...
How to use it:
  ...
  - use CCLoader to write the binary to de device: make program or make program_mini if you have the smaller lock

Once the firmware is on the lock, reassemble it.

To use the lock: open the shackle and press the center button both red and blue lights will blink. If you hold the button down for as long as the red and blue lights blink the lock will go into random mode - and the time you set is the *maximum* time. In random mode a random duration in minutes between 0 and the time you set will be used.
...
If the battery is too low to close and open the lock for the duration you set - the lock will not lock. If this happens after you try twice, replace the battery. 
